...
window.m_prefix = "m.star365.com_";
...
var addWare = function (b, g, f, c, j, d) {
    if (isNotBlank(f) && f) {
        var h = "cart/addDirOrder@pId=" + b + "&num=" + g + "&type=" + c;
        var l = ajax(h,
        function () {
        ...
        var i = "cart/add@pId=" + b + "&num=" + g + "&type=" + c;
        var l = ajax(i,
        function (o) {
            e.stop();
            $("#spinner2").hide();
...
var deSelectedAll = function (c) {
    var a = "cart/check@checked=" + 8;
    var b = ajax(a, c)
};
var xmlHttp;
...
function cancelHotWord() { $("#newkeyword").val(""); $("#newkeyword").focus() }
...
$(function () {
    ...
    if ($("#btnHwkey")) {
        $("#btnHwkey").click(function () {
            if ($("#hwkey").css("display") == "none") {
                $("#hwkey").show()
            }
            else {
                $("#hwkey").hide()
    ...
    if (a.indexOf("/") < 0 || (a.split("/").length <= 2 && a.indexOf("/index") >= 0)) {
        $("#hwkey .new-tbl-cell").eq(0).children().addClass("on")
    }
    if (a.indexOf("/category/all.html") > -1) {
        $("#hwkey .new-tbl-cell").eq(1).children().addClass("on")
    }
    if (a.indexOf("/cart/cart.html") > -1) {
        $("#hwkey .new-tbl-cell").eq(2).children().addClass("on")
    }
    if (a.indexOf("/user/") > -1) {
        $("#hwkey .new-tbl-cell").eq(3).children().addClass("on")
    }
    if ($("#btnHwBox")) {
        $("#btnHwBox").click(function () {
            $("#_hwBox").hide();
            $("#_hwSearch").show();
            $("#newkeyword").focus()
        })
